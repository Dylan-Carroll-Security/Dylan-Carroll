 ...
 * -------------------------------------------------------------
 */
tokyo_nul bErrorPrint(tokyo_chr* fmt, ...);
tokyo_nul displayBuffer(Buffer* ptr_Buffer);
tokyo_lng getFileSize(tokyo_chr* fname);
tokyo_int isNumber(const tokyo_chr* ns);
tokyo_nul startBuffer(tokyo_chr*, tokyo_chr*, tokyo_chr, tokyo_int, tokyo_int);
...
tokyo_int mainBuffer(tokyo_int argc, tokyo_chr** argv) {
	...
	tokyo_chr* program = argv[0];
	tokyo_chr* input = argv[2];
	tokyo_chr mode = MODE_FIXED;
	tokyo_int isAnsiC = !ANSI_C;		/* ANSI C flag */
	tokyo_int size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
void startBuffer(tokyo_chr* program, tokyo_chr* input, tokyo_chr mode, tokyo_int size, tokyo_int increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	tokyo_int loadSize = 0;			/* the size of the file loaded in the buffer */
	tokyo_chr symbol;				/* symbol read from input file */
