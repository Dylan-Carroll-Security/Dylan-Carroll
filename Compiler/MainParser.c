...
tokyo_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern tokyo_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern tokyo_int line; /* source code line number - defined in scanner.c */
...
extern tokyo_nul startParser(void);
extern tokyo_int startScanner(BufferPointer sc_buf);

static tokyo_nul printParserError(char* fmt, ...);
static tokyo_nul displayParser(BufferPointer ptrBuffer);
static tokyo_lng getParserFileSize(char* fname);
static tokyo_nul callGarbageCollector(void);
...
tokyo_nul printParserError(tokyo_chr* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(tokyo_nul)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
tokyo_lng getParserFileSize(tokyo_chr* fname) {
	FILE* input;
	tokyo_lng flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
tokyo_nul displayParser(Buffer* ptrBuffer) {
...
tokyo_nul callGarbageCollector(tokyo_nul) {
