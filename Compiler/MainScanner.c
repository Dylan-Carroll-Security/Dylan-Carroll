...
int i;
/* External objects */
extern tokyo_int line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(tokyo_nul);
 ...
 * -------------------------------------------------------------
 */
tokyo_nul printScannerError(tokyo_chr* fmt, ...);
tokyo_nul displayScanner(Buffer* ptrBuffer);
tokyo_lng getScannerFilesize(tokyo_chr* fname);
...
tokyo_int mainScanner(tokyo_int argc, tokyo_chr** argv) {
	...
	extern char matchedTokens;       // MATCHED TOKENS ------------------------------------s
